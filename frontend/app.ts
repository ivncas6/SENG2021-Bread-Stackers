...
type WorkspaceMode = 'buying' | 'selling';
...
type AddressIdResponse = JsonRecord & {
  addressId?: number;
};

type OrgIdResponse = JsonRecord & {
  orgId?: number;
};

type OrderIdResponse = JsonRecord & {
  orderId?: string;
};

type SignedUrlResponse = JsonRecord & {
  signedUrl?: string;
};

type OrgMembersResponse = JsonRecord & {
  users?: OrgMember[];
};

type AddressesResponse = JsonRecord & {
  addresses?: AddressRecord[];
};

type OrganisationsResponse = JsonRecord & {
  organisations?: KnownOrg[];
};
...
type CatalogueResponse = JsonRecord & {
  items?: CatalogueItem[];
};
...
type SellerOrderSummary = OrderSummary & {
  buyerOrgID?: number;
};

type SellerOrdersResponse = JsonRecord & {
  orders?: SellerOrderSummary[];
};

type SellerOrderDetail = JsonRecord & {
  address?: string;
  buyerOrgId?: number;
  currency?: string;
  deliveryAddressId?: number | null;
...
type AiChatResponse = JsonRecord & {
...
const publicViews: ViewName[] = ['landing', 'login', 'register'];
const organisationViews: ViewName[] = ['organisation', 'addresses', 'members'];
const buyingViews: ViewName[] = [
  'marketplace',
  'dashboard',
  'orderDetail',
  'createOrder',
...
const sellerViews: ViewName[] = [
  'sellerDashboard',
  'catalogue',
  'sellerOrders',
  'sellerOrderDetail',
];
const orgRequiredViews: ViewName[] = [
  'organisation',
  'addresses',
  'members',
  'marketplace',
...
let aiStreamingReply = '';
let aiTypingTimeoutId: number | undefined;
let currentOrderDetail: OrderDetail | null = null;
let currentMembers: OrgMember[] = [];
let currentOrgRole: OrgRole | null = null;
let currentMarketplaceOrgs: KnownOrg[] = [];
let currentSellerCatalogue: CatalogueItem[] = [];
let marketplaceSelectedSellerId: number | null = null;
let marketplaceCartItems: MarketplaceCartItem[] = [];
let currentSellerOrderDetail: SellerOrderDetail | null = null;
let currentWorkspaceMode: WorkspaceMode = 'buying';
...
function getAiHistoryStorageKey(orgId: number): string {
  return `${getScopedStorageKey(storageKeys.aiHistoryPrefix)}.${orgId}`;
}

function getKnownOrgs(): KnownOrg[] {
  ...
  try {
    const parsed = JSON.parse(raw) as KnownOrg[];
...
function saveKnownOrgs(orgs: KnownOrg[]): void {
...
function getStoredOrgRoles(): Record<string, OrgRole> {
  ...
  try {
    const parsed = JSON.parse(raw) as Record<string, OrgRole>;
...
function saveStoredOrgRoles(roles: Record<string, OrgRole>): void {
  localStorage.setItem(
...
function rememberOrgRole(orgId: number, role: OrgRole): void {
  const roles = getStoredOrgRoles();
  roles[String(orgId)] = role;
  saveStoredOrgRoles(roles);
}

function getStoredOrgRole(orgId: number | null): OrgRole | null {
  if (!orgId) {
    return null;
  }
...
function getStoredWorkspaceModes(): Record<string, WorkspaceMode> {
  ...
  try {
    const parsed = JSON.parse(raw) as Record<string, WorkspaceMode>;
    return parsed && typeof parsed === 'object' ? parsed : {};
  } catch {
    return {};
  }
...
function saveStoredWorkspaceModes(modes: Record<string, WorkspaceMode>): void {
  localStorage.setItem(
...
function getStoredWorkspaceMode(orgId: number | null): WorkspaceMode {
  if (!orgId) {
    return 'buying';
  }
...
function rememberWorkspaceMode(orgId: number, mode: WorkspaceMode): void {
  const modes = getStoredWorkspaceModes();
  modes[String(orgId)] = mode;
  saveStoredWorkspaceModes(modes);
}

function syncCurrentWorkspaceMode(): void {
...
function rememberKnownOrg(org: KnownOrg): void {
...
function removeKnownOrg(orgId: number): void {
  const next = getKnownOrgs().filter((org) => org.orgId !== orgId);
  saveKnownOrgs(next);
}

function getActiveOrgId(): number | null {
...
function setActiveOrgId(orgId: number | null): void {
...
function ensureActiveOrgSelection(): number | null {
  const activeOrgId = getActiveOrgId();
...
function getActiveOrg(): KnownOrg | null {
...
function isSellerRole(role: OrgRole | null): boolean {
  return role === 'OWNER' || role === 'ADMIN';
}

function getCurrentViewName(): ViewName {
  const entry = (Object.entries(viewElements) as Array<[ViewName, HTMLElement]>)
    .find(([, element]) => element.classList.contains('is-active'));

  return entry?.[0] || 'landing';
}

function getKnownAddresses(): AddressRecord[] {
  ...
  try {
    ...
    return Array.isArray(parsed)
      ? parsed.filter((address) => Number.isInteger(address.addressID))
      : [];
  } catch {
    return [];
  }
...
function saveKnownAddresses(addresses: AddressRecord[]): void {
  localStorage.setItem(
...
function rememberAddress(address: AddressRecord): void {
...
function mergeAddresses(addresses: AddressRecord[]): void {
  addresses.forEach((address) => rememberAddress(address));
}

function removeKnownAddress(addressId: number): void {
  saveKnownAddresses(
    getKnownAddresses().filter((address) => address.addressID !== addressId),
  );
}

function normaliseAiChatHistory(value: unknown): AiChatMessage[] {
...
function getStoredAiHistory(orgId: number): AiChatMessage[] {
...
function saveAiHistory(orgId: number, messages: AiChatMessage[]): void {
  localStorage.setItem(getAiHistoryStorageKey(orgId), JSON.stringify(messages));
}

function clearAiHistory(orgId: number): void {
...
function setBannerMessage(element: HTMLElement, message: string): void {
  if (!message) {
...
function formatDateTimeInput(epochMillis: number | undefined): string {
  if (!epochMillis) {
    return '';
  }
...
function buildEpochFromInput(value: string): number {
...
function normaliseAssistantText(value: string): string {
  return value
    .replace(/\r\n/g, '\n')
    .replace(/\*\*(.*?)\*\*/g, '$1')
    .replace(/^#{1,6}\s+/gm, '')
...
function getMessageDisplayText(message: AiChatMessage): string {
  return message.role === 'assistant'
    ? normaliseAssistantText(message.content)
    : message.content;
}

function stopAiTypingAnimation(): void {
...
function resetAiInputHeight(): void {
  elements.aiChatInput.style.height = 'auto';
  elements.aiChatInput.style.overflowY = 'hidden';
}

function resizeAiChatInput(): void {
  const maxHeight = 132;
  elements.aiChatInput.style.height = 'auto';

  const nextHeight = Math.min(elements.aiChatInput.scrollHeight, maxHeight);
  elements.aiChatInput.style.height = `${nextHeight}px`;
  elements.aiChatInput.style.overflowY =
    elements.aiChatInput.scrollHeight > maxHeight ? 'auto' : 'hidden';
}

async function animateAssistantReply(fullReply: string): Promise<void> {
...
function formatAddress(address: AddressRecord): string {
  const parts = [
    address.street,
    address.city || '',
    address.postcode || '',
...
function getOrgLabel(org: KnownOrg | null): string {
  if (!org) {
...
function syncCurrentOrgRole(): void {
  const activeOrgId = getActiveOrgId();
...
function clearOrderDetailPanel(): void {
  currentOrderDetail = null;
  closeCancelOrderModal();
  elements.detailOrderHeading.textContent = 'Order detail';
  elements.detailOrderMeta.textContent =
    'Review the purchase order, then download its UBL or update the order.';
...
function clearSellerOrderDetailPanel(): void {
  currentSellerOrderDetail = null;
  elements.sellerDetailOrderHeading.textContent = 'Order request';
  elements.sellerDetailOrderMeta.textContent =
    'Review the buyer request and decide whether to accept or reject it.';
...
function updateNavigationState(activeView: ViewName): void {
  document.querySelectorAll<HTMLElement>('[data-nav-target]').forEach((button) => {
    ...
    const groupedActive = (
      (activeView === 'orderDetail' || activeView === 'updateOrder') && target === 'dashboard'
    ) || (
      activeView === 'sellerOrderDetail' && target === 'sellerOrders'
    );
...
function updateAuthChrome(): void {
  ...
  elements.activeOrgBadge.textContent = hasActiveOrg
    ? getOrgLabel(activeOrg)
    : 'Setup required';
  ...
  document.querySelectorAll<HTMLElement>('[data-requires-org]').forEach((element) => {
    element.classList.toggle('hidden', !authenticated || !hasActiveOrg);
  });

  document.querySelectorAll<HTMLElement>('[data-workspace]').forEach((element) => {
    const workspace = element.dataset.workspace as WorkspaceMode | undefined;
  ...
  document.querySelectorAll<HTMLElement>('[data-requires-no-org]').forEach((element) => {
    element.classList.toggle('hidden', !authenticated || hasActiveOrg);
  });
...
function populateOrgSelectors(): void {
  const knownOrgs = getKnownOrgs();
  const activeOrgId = getActiveOrgId();
  const activeOrg = getActiveOrg();
  const orgOptions = knownOrgs.length > 0
    ? knownOrgs.map((org) => `
  ...
  elements.createOrderOrgName.value = activeOrg
    ? getOrgLabel(activeOrg)
...
function populateAddressSelectors(selectedOrderAddressId?: number | null): void {
...
function renderAddressBook(): void {
  const addresses = getKnownAddresses();
...
function renderOrderDetail(detail: OrderDetail): void {
  ...
  elements.detailOrderMeta.textContent =
    `${detail.status || 'Unknown'} • ${detail.issuedDate || 'No issue date'}`;
...
function getMarketplaceSelectedSeller(): KnownOrg | null {
...
function calculateMarketplaceCartTotals(): { subtotal: number; tax: number; total: number } {
...
function renderMarketplaceSellerList(): void {
...
function renderMarketplaceCatalogue(): void {
  const selectedSeller = getMarketplaceSelectedSeller();

  elements.marketplaceSelectedSellerName.textContent = selectedSeller
    ? selectedSeller.orgName
    : 'Select a seller';
  elements.marketplaceSelectedSellerMeta.textContent = selectedSeller
    ...
    : 'Pick a seller from the left to load their active catalogue items.';
  elements.marketplaceCheckoutSellerName.value = selectedSeller
    ? getOrgLabel(selectedSeller)
    : 'No seller selected';
...
function renderMarketplaceCart(): void {
  ...
  elements.marketplacePlaceOrderButton.disabled =
    marketplaceCartItems.length === 0 || !getMarketplaceSelectedSeller();
}

function validateMarketplaceCheckoutForm(form: FormData): {
...
function renderCatalogueManager(): void {
...
function renderSellerDashboardSummary(
  _catalogueItems: CatalogueItem[],
...
function renderSellerOrders(orders: SellerOrderSummary[]): void {
  if (!getActiveOrgId()) {
...
function renderSellerOrderDetail(detail: SellerOrderDetail): void {
...
function setAiChatStatus(message: string): void {
  if (!message) {
...
function updateAiSuggestionsVisibility(): void {
  const activeOrg = getActiveOrg();
  const shouldShowSuggestions = Boolean(activeOrg)
...
function setAiChatPending(isPending: boolean): void {
...
function renderAiChatHistory(): void {
  const activeOrg = getActiveOrg();
...
function loadAiChatForActiveOrg(): void {
  const activeOrgId = getActiveOrgId();
...
function formatDisplayDateTime(epochMillis: number | undefined): string {
  if (!epochMillis) {
    return 'N/A';
  }
...
function updateCreateOrderSummary(): void {
  ...
  elements.createOrderSummaryLines.innerHTML = meaningfulItems.length > 0
    ? meaningfulItems
      ...
      .join('')
    : '<p class="summary-empty">Add items to see the order breakdown.</p>';
...
function populateReadonlyFields(): void {
  const profile = getStoredProfile();
  const activeOrg = getActiveOrg();
...
function prepareCreateOrderView(): void {
  const activeOrg = getActiveOrg();
  const addresses = getKnownAddresses();
...
function prepareUpdateOrderView(detail: OrderDetail): void {
...
async function loadCatalogueForSeller(orgId: number): Promise<void> {
  const payload = await apiRequest<CatalogueResponse>(`/v3/organisation/${orgId}/catalogue`);
...
async function loadMarketplaceData(): Promise<void> {
  ...
  const payload = await apiRequest<OrganisationsResponse>('/v3/organisations');
  currentMarketplaceOrgs = (payload.organisations || [])
...
async function loadCatalogueManager(): Promise<void> {
  const activeOrgId = getActiveOrgId();
  ...
  setBannerMessage(
    elements.catalogueNotice,
    isSellerRole(currentOrgRole)
      ? `Managing catalogue items for ${getOrgLabel(getActiveOrg())}.`
      : `Viewing catalogue items for ${getOrgLabel(getActiveOrg())}.`,
  );
  await loadCatalogueForSeller(activeOrgId);
  renderCatalogueManager();
}

async function loadSellerOrders(): Promise<void> {
  const activeOrgId = getActiveOrgId();
  ...
  const path = status
    ? `/v3/organisation/${activeOrgId}/orders/received?status=${encodeURIComponent(status)}`
    : `/v3/organisation/${activeOrgId}/orders/received`;
  const payload = await apiRequest<SellerOrdersResponse>(path);
  renderSellerOrders(payload.orders || []);
}

async function loadSellerDashboardData(): Promise<void> {
  const activeOrgId = getActiveOrgId();
  ...
  const results = await Promise.allSettled([
    apiRequest<CatalogueResponse>(`/v3/organisation/${activeOrgId}/catalogue`),
    apiRequest<SellerOrdersResponse>(`/v3/organisation/${activeOrgId}/orders/received`),
  ]);
...
async function loadSellerOrderDetail(orderId: string): Promise<void> {
  const activeOrgId = getActiveOrgId();
  ...
  const payload = await apiRequest<SellerOrderDetail>(
...
async function openSellerOrderDetailView(orderId: string): Promise<void> {
  try {
...
async function performSellerOrderAction(
  orderId: string,
...
async function fetchAddressById(addressId: number): Promise<AddressRecord> {
  return apiRequest<AddressRecord>(`/v2/address/${addressId}`);
}
...
async function refreshWorkspaceData(): Promise<void> {
  ...
  const loaders = await Promise.allSettled([
    apiRequest<AddressesResponse>(`/v2/organisation/${activeOrgId}/address`),
    apiRequest<OrgMembersResponse>(`/v2/organisation/${activeOrgId}/members`),
  ]);
...
async function loadOrders(): Promise<void> {
  ...
  try {
    const payload = await apiRequest<{ orders?: OrderSummary[] }>(
...
async function loadOrderDetail(orderId: string): Promise<void> {
  ...
  try {
    const payload = await apiRequest<OrderDetail>(
...
async function openOrderDetailView(orderId: string): Promise<void> {
...
async function openUpdateOrderView(orderId?: string): Promise<void> {
  if (orderId) {
    await loadOrderDetail(orderId);
  }
...
function buildUpdateOrderPayload(form: FormData): JsonRecord {
  const deliveryAddressId = parseInteger(getFormValue(form, 'deliveryAddressId'));
...
function getAuthenticatedHomeView(): ViewName {
  if (!getActiveOrg()) {
    return 'orgChoice';
  }
...
const elements = {
  acceptReceivedOrderButton: getRequiredElement<HTMLButtonElement>('acceptReceivedOrderButton'),
  activeOrgBadge: getRequiredElement<HTMLElement>('activeOrgBadge'),
  activeOrgLabel: getRequiredElement<HTMLInputElement>('activeOrgLabel'),
  activeOrgSelect: getRequiredElement<HTMLSelectElement>('activeOrgSelect'),
  addMemberForm: getRequiredElement<HTMLFormElement>('addMemberForm'),
  addOrderItemButton: getRequiredElement<HTMLButtonElement>('addOrderItemButton'),
  aiChatForm: getRequiredElement<HTMLFormElement>('aiChatForm'),
  aiChatHistory: getRequiredElement<HTMLElement>('aiChatHistory'),
  aiChatInput: getRequiredElement<HTMLTextAreaElement>('aiChatInput'),
  aiChatResetButton: getRequiredElement<HTMLButtonElement>('aiChatResetButton'),
  aiChatSendButton: getRequiredElement<HTMLButtonElement>('aiChatSendButton'),
  aiChatSuggestions: getRequiredElement<HTMLElement>('aiChatSuggestions'),
  aiChatStatus: getRequiredElement<HTMLElement>('aiChatStatus'),
  aiWidgetCloseButton: getRequiredElement<HTMLButtonElement>('aiWidgetCloseButton'),
  aiWidgetLauncher: getRequiredElement<HTMLButtonElement>('aiWidgetLauncher'),
  aiWidgetPanel: getRequiredElement<HTMLElement>('aiWidgetPanel'),
  addressesList: getRequiredElement<HTMLElement>('addressesList'),
  brandText: getRequiredElement<HTMLElement>('brandText'),
  cancelOrderButton: getRequiredElement<HTMLButtonElement>('cancelOrderButton'),
  cancelOrderForm: getRequiredElement<HTMLFormElement>('cancelOrderForm'),
  cancelOrderModal: getRequiredElement<HTMLElement>('cancelOrderModal'),
  cancelOrderReason: getRequiredElement<HTMLTextAreaElement>('cancelOrderReason'),
  cancelOrderSubtitle: getRequiredElement<HTMLElement>('cancelOrderSubtitle'),
  connectOrganisationForm: getRequiredElement<HTMLFormElement>('connectOrganisationForm'),
  createAddressForm: getRequiredElement<HTMLFormElement>('createAddressForm'),
  createOrderAddressId: getRequiredElement<HTMLSelectElement>('createOrderAddressId'),
  createOrderBuyerEmail: getRequiredElement<HTMLInputElement>('createOrderBuyerEmail'),
  ...
  createOrderNotice: getRequiredElement<HTMLElement>('createOrderNotice'),
  createOrderOrgName: getRequiredElement<HTMLInputElement>('createOrderOrgName'),
  createCatalogueItemForm: getRequiredElement<HTMLFormElement>('createCatalogueItemForm'),
  createOrgAddressId: getRequiredElement<HTMLSelectElement>('createOrgAddressId'),
  createOrgSetupForm: getRequiredElement<HTMLFormElement>('createOrgSetupForm'),
  createOrganisationForm: getRequiredElement<HTMLFormElement>('createOrganisationForm'),
  catalogueCancelComposerButton:
    getRequiredElement<HTMLButtonElement>('catalogueCancelComposerButton'),
  catalogueCreateCard: getRequiredElement<HTMLElement>('catalogueCreateCard'),
  catalogueCreateNotice: getRequiredElement<HTMLElement>('catalogueCreateNotice'),
  catalogueItemsList: getRequiredElement<HTMLElement>('catalogueItemsList'),
  catalogueNotice: getRequiredElement<HTMLElement>('catalogueNotice'),
  dashboardCreateOrderButton: getRequiredElement<HTMLButtonElement>('dashboardCreateOrderButton'),
  deleteOrganisationButton: getRequiredElement<HTMLButtonElement>('deleteOrganisationButton'),
  detailOrderHeading: getRequiredElement<HTMLElement>('detailOrderHeading'),
  detailOrderMeta: getRequiredElement<HTMLElement>('detailOrderMeta'),
  dismissCancelOrderButton: getRequiredElement<HTMLButtonElement>('dismissCancelOrderButton'),
  downloadUblButton: getRequiredElement<HTMLButtonElement>('downloadUblButton'),
  feedbackBanner: getRequiredElement<HTMLElement>('feedbackBanner'),
  goToWorkspaceFromOrders: getRequiredElement<HTMLButtonElement>('goToWorkspaceFromOrders'),
  landingLoginButton: getRequiredElement<HTMLButtonElement>('landingLoginButton'),
  landingRegisterButton: getRequiredElement<HTMLButtonElement>('landingRegisterButton'),
  loginForm: getRequiredElement<HTMLFormElement>('loginForm'),
  loginToRegisterButton: getRequiredElement<HTMLButtonElement>('loginToRegisterButton'),
  logoutButton: getRequiredElement<HTMLButtonElement>('logoutButton'),
  manageMenuToggle: getRequiredElement<HTMLElement>('manageMenuToggle'),
  marketplaceCheckoutAddressId:
    getRequiredElement<HTMLSelectElement>('marketplaceCheckoutAddressId'),
  marketplaceCheckoutForm: getRequiredElement<HTMLFormElement>('marketplaceCheckoutForm'),
  marketplaceCheckoutNotice: getRequiredElement<HTMLElement>('marketplaceCheckoutNotice'),
  marketplaceCheckoutSellerName:
    getRequiredElement<HTMLInputElement>('marketplaceCheckoutSellerName'),
  marketplaceCatalogueList: getRequiredElement<HTMLElement>('marketplaceCatalogueList'),
  marketplaceCartLines: getRequiredElement<HTMLElement>('marketplaceCartLines'),
  marketplaceDeliveryEnd: getRequiredElement<HTMLInputElement>('marketplaceDeliveryEnd'),
  marketplaceDeliveryStart: getRequiredElement<HTMLInputElement>('marketplaceDeliveryStart'),
  marketplaceNotice: getRequiredElement<HTMLElement>('marketplaceNotice'),
  marketplacePlaceOrderButton: getRequiredElement<HTMLButtonElement>('marketplacePlaceOrderButton'),
  marketplaceSelectedSellerMeta: getRequiredElement<HTMLElement>('marketplaceSelectedSellerMeta'),
  marketplaceSelectedSellerName: getRequiredElement<HTMLElement>('marketplaceSelectedSellerName'),
  marketplaceSellerList: getRequiredElement<HTMLElement>('marketplaceSellerList'),
  marketplaceSubtotal: getRequiredElement<HTMLElement>('marketplaceSubtotal'),
  marketplaceTax: getRequiredElement<HTMLElement>('marketplaceTax'),
  marketplaceTotal: getRequiredElement<HTMLElement>('marketplaceTotal'),
  membersList: getRequiredElement<HTMLElement>('membersList'),
  openUpdateOrderButton: getRequiredElement<HTMLButtonElement>('openUpdateOrderButton'),
  ...
  ordersNotice: getRequiredElement<HTMLElement>('ordersNotice'),
  privateNav: getRequiredElement<HTMLElement>('privateNav'),
  publicNav: getRequiredElement<HTMLElement>('publicNav'),
  refreshWorkspaceButton: getRequiredElement<HTMLButtonElement>('refreshWorkspaceButton'),
  registerForm: getRequiredElement<HTMLFormElement>('registerForm'),
  registerToLoginButton: getRequiredElement<HTMLButtonElement>('registerToLoginButton'),
  rejectReceivedOrderButton: getRequiredElement<HTMLButtonElement>('rejectReceivedOrderButton'),
  sellerAcceptedCount: getRequiredElement<HTMLElement>('sellerAcceptedCount'),
  sellerActiveProductCount: getRequiredElement<HTMLElement>('sellerActiveProductCount'),
  sellerDashboardNotice: getRequiredElement<HTMLElement>('sellerDashboardNotice'),
  sellerDashboardOrgLabel: getRequiredElement<HTMLElement>('sellerDashboardOrgLabel'),
  sellerDetailOrderHeading: getRequiredElement<HTMLElement>('sellerDetailOrderHeading'),
  sellerDetailOrderMeta: getRequiredElement<HTMLElement>('sellerDetailOrderMeta'),
  sellerOrderInfoOutput: getRequiredElement<HTMLElement>('sellerOrderInfoOutput'),
  sellerOrderNotice: getRequiredElement<HTMLElement>('sellerOrderNotice'),
  sellerOrdersEmptyState: getRequiredElement<HTMLElement>('sellerOrdersEmptyState'),
  sellerOrdersList: getRequiredElement<HTMLElement>('sellerOrdersList'),
  sellerOrdersNotice: getRequiredElement<HTMLElement>('sellerOrdersNotice'),
  sellerOrdersStatusFilter: getRequiredElement<HTMLSelectElement>('sellerOrdersStatusFilter'),
  sellerPendingCount: getRequiredElement<HTMLElement>('sellerPendingCount'),
  ...
  updateDeliveryEnd: getRequiredElement<HTMLInputElement>('updateDeliveryEnd'),
  updateDeliveryStart: getRequiredElement<HTMLInputElement>('updateDeliveryStart'),
  updateOrderAddressId: getRequiredElement<HTMLSelectElement>('updateOrderAddressId'),
  updateOrderForm: getRequiredElement<HTMLFormElement>('updateOrderForm'),
  updateOrderHeading: getRequiredElement<HTMLElement>('updateOrderHeading'),
  updateOrderNotice: getRequiredElement<HTMLElement>('updateOrderNotice'),
  updateOrderStatus: getRequiredElement<HTMLSelectElement>('updateOrderStatus'),
  updateOrgAddressId: getRequiredElement<HTMLSelectElement>('updateOrgAddressId'),
  updateOrganisationForm: getRequiredElement<HTMLFormElement>('updateOrganisationForm'),
  updateOrgName: getRequiredElement<HTMLInputElement>('updateOrgName'),
  welcomeName: getRequiredElement<HTMLElement>('welcomeName'),
  workspaceBuyingButton: getRequiredElement<HTMLButtonElement>('workspaceBuyingButton'),
  workspaceSellingButton: getRequiredElement<HTMLButtonElement>('workspaceSellingButton'),
  workspaceSwitcher: getRequiredElement<HTMLElement>('workspaceSwitcher'),
  workspaceNotice: getRequiredElement<HTMLElement>('workspaceNotice'),
} as const;

const viewElements: Record<ViewName, HTMLElement> = {
  addresses: getRequiredElement<HTMLElement>('addressesView'),
  catalogue: getRequiredElement<HTMLElement>('catalogueView'),
  createOrg: getRequiredElement<HTMLElement>('createOrgView'),
  createOrder: getRequiredElement<HTMLElement>('createOrderView'),
  dashboard: getRequiredElement<HTMLElement>('dashboardView'),
  joinOrg: getRequiredElement<HTMLElement>('joinOrgView'),
  landing: getRequiredElement<HTMLElement>('landingView'),
  login: getRequiredElement<HTMLElement>('loginView'),
  marketplace: getRequiredElement<HTMLElement>('marketplaceView'),
  members: getRequiredElement<HTMLElement>('membersView'),
  organisation: getRequiredElement<HTMLElement>('organisationView'),
  orgChoice: getRequiredElement<HTMLElement>('orgChoiceView'),
  orderDetail: getRequiredElement<HTMLElement>('orderDetailView'),
  register: getRequiredElement<HTMLElement>('registerView'),
  sellerDashboard: getRequiredElement<HTMLElement>('sellerDashboardView'),
  sellerOrderDetail: getRequiredElement<HTMLElement>('sellerOrderDetailView'),
  sellerOrders: getRequiredElement<HTMLElement>('sellerOrdersView'),
  updateOrder: getRequiredElement<HTMLElement>('updateOrderView'),
};
...
elements.aiChatForm.addEventListener('submit', async (event: SubmitEvent) => {
  event.preventDefault();
  ...
  try {
    const payload = await apiRequest<AiChatResponse>(
    ...
    const assistantReply = nextMessages[nextMessages.length - 1]?.role === 'assistant'
      ? nextMessages[nextMessages.length - 1].content
      : payload.reply || 'The assistant did not return a response.';

    await animateAssistantReply(assistantReply);
    aiChatHistory = nextMessages.length > 0
      ? nextMessages
      : [
        ...previousHistory,
...
elements.activeOrgSelect.addEventListener('change', async () => {
  ...
  const nextView = sellerViews.includes(currentView) && !isSellerRole(currentOrgRole)
    ? getAuthenticatedHomeView()
    : currentView;

  await showView(nextView);
});

elements.connectOrganisationForm.addEventListener('submit', async (event: SubmitEvent) => {
  event.preventDefault();
  const form = getFormData(event.currentTarget);
...
elements.createOrgSetupForm.addEventListener('submit', async (event: SubmitEvent) => {
  event.preventDefault();
  const form = getFormData(event.currentTarget);
  const orgName = getFormValue(form, 'orgName');

  try {
    const addressPayload = await apiRequest<AddressIdResponse>('/v2/address', {
      body: {
        city: getFormValue(form, 'city') || undefined,
        country: getFormValue(form, 'country') || 'AUS',
        postcode: getFormValue(form, 'postcode') || undefined,
    ...
    const organisationPayload = await apiRequest<OrgIdResponse>('/v2/organisation', {
      body: {
    ...
    const countryField = document.getElementById('setupCountry') as HTMLInputElement | null;
...
elements.createAddressForm.addEventListener('submit', async (event: SubmitEvent) => {
  event.preventDefault();
  const form = getFormData(event.currentTarget);

  try {
    const payload = await apiRequest<AddressIdResponse>('/v2/address', {
      body: {
        city: getFormValue(form, 'city') || undefined,
        country: getFormValue(form, 'country') || 'AUS',
        postcode: getFormValue(form, 'postcode') || undefined,
    ...
    const countryField = document.getElementById('addressCountry') as HTMLInputElement | null;
...
elements.createOrganisationForm.addEventListener('submit', async (event: SubmitEvent) => {
  event.preventDefault();
  const form = getFormData(event.currentTarget);
  ...
  try {
    const payload = await apiRequest<OrgIdResponse>('/v2/organisation', {
      body: {
        addressId,
        orgName,
      },
...
elements.updateOrganisationForm.addEventListener('submit', async (event: SubmitEvent) => {
  event.preventDefault();
  const activeOrg = getActiveOrg();
  const form = getFormData(event.currentTarget);
  const addressId = parseInteger(getFormValue(form, 'addressId'));
  ...
  try {
    await apiRequest<OrgIdResponse>(`/v2/organisation/${activeOrg.orgId}`, {
      body: {
        addressId,
        orgName,
      },
...
elements.addMemberForm.addEventListener('submit', async (event: SubmitEvent) => {
  event.preventDefault();
  const activeOrg = getActiveOrg();
  const form = getFormData(event.currentTarget);
  const email = getFormValue(form, 'email');
...
elements.createCatalogueItemForm.addEventListener('submit', async (event: SubmitEvent) => {
  event.preventDefault();
  const activeOrgId = getActiveOrgId();
  ...
  const submitButton = elements.createCatalogueItemForm.querySelector<HTMLButtonElement>(
    'button[type="submit"]',
  );
...
elements.marketplaceCheckoutForm.addEventListener('submit', async (event: SubmitEvent) => {
  event.preventDefault();
  const activeOrgId = getActiveOrgId();
  const selectedSeller = getMarketplaceSelectedSeller();
  ...
  try {
    ...
    const { deliveryAddressId, deliveryStart, deliveryEnd } =
      validateMarketplaceCheckoutForm(form);

    const response = await apiRequest<OrderIdResponse>(`/v3/organisation/${activeOrgId}/order`, {
      body: {
        deliveryAddressId,
    ...
    setBannerMessage(
      ...
      response.orderId
        ? `Order ${response.orderId} submitted for seller approval.`
        : 'Order request submitted for seller approval.',
    );
    showFeedback(
      response.orderId
        ? `Order ${response.orderId} submitted for seller approval.`
...
elements.createOrderForm.addEventListener('submit', async (event: SubmitEvent) => {
  ...
  try {
    ...
    const response = await apiRequest<OrderIdResponse>(
...
elements.downloadUblButton.addEventListener('click', async () => {
  ...
  try {
    const payload = await apiRequest<SignedUrlResponse>(
...
document.addEventListener('click', async (event) => {
  ...
  const viewButton = target.closest<HTMLElement>('[data-order-view]');
  ...
  const updateButton = target.closest<HTMLElement>('[data-order-update]');
  ...
  const marketplaceSellerButton = target.closest<HTMLElement>('[data-marketplace-seller]');
  ...
  const marketplaceAddButton = target.closest<HTMLElement>('[data-marketplace-add]');
  ...
  const marketplaceRemoveButton = target.closest<HTMLElement>('[data-marketplace-remove]');
  ...
  const sellerOrderViewButton = target.closest<HTMLElement>('[data-seller-order-view]');
  ...
  const sellerOrderAcceptButton = target.closest<HTMLElement>('[data-seller-order-accept]');
  ...
  const sellerOrderRejectButton = target.closest<HTMLElement>('[data-seller-order-reject]');
  ...
  const catalogueEditButton = target.closest<HTMLElement>('[data-catalogue-edit]');
  ...
  const catalogueDeleteButton = target.closest<HTMLElement>('[data-catalogue-delete]');
  ...
  const suggestionButton = target.closest<HTMLElement>('[data-ai-suggestion]');
  ...
  const memberSaveButton = target.closest<HTMLElement>('[data-member-save]');
  ...
  const memberRemoveButton = target.closest<HTMLElement>('[data-member-remove]');
  ...
  const addressEditButton = target.closest<HTMLElement>('[data-address-edit]');
  ...
  const addressDeleteButton = target.closest<HTMLElement>('[data-address-delete]');
